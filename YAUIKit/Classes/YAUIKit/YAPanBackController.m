...
#define BACKGROUND_VIEW_TAG 12121
...
- (void) handlePanBackGR:(UIPanGestureRecognizer *)gestureRecongnizer {
  ...
  if (gestureRecongnizer.state == UIGestureRecognizerStateBegan) {
    ...
    [UIView animateWithDuration:.2 animations:^{
      ...
      if (backSuccess && _currentViewController) {
        [_currentViewController.navigationController.view layoutIfNeeded];
...
- (void) removePanBackFromView:(UIView *)view {
   if (_toViewController && (((UIView *)[_toViewController.view.subviews lastObject]).tag == BACKGROUND_VIEW_TAG)) {
      _completionBlock(_currentViewController, _toViewController, YES);
   }
